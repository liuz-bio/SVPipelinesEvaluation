...
class EvaluationSV():
    ...
    def writeReslut(self, precision_GT,recall_GT,F1_GT,precision_no_GT,recall_no_GT,F1_no_GT):
        ...
        with open('%s/summary.txt'%self.outFile,'w') as out:
            ...
            out.write('TP_GT_comm,'+str(len(set(self.TP_GT_comm)))+'\n')
            out.write('TP_GT_base,'+str(len(set(self.TP_GT_base)))+'\n')
            out.write('TP_no_GT_comm,'+str(len(set(self.TP_no_GT_comm)))+'\n')
            out.write('TP_no_GT_base,'+str(len(set(self.TP_no_GT_base)))+'\n')
            out.write('commSize,'+str(self.commSize)+'\n')
            out.write('baseSize,'+str(self.baseSize)+'\n')
        ...
        with open('%s/lsv.txt'%self.outFile,'w') as out:
            ...
            out.write(','.join(['ID','base_GT','comm_GT','base_no_GT','comm_no_GT',
                                ...
                                'no_GT_precision','no_GT_recall','no_GT_F1'])+'\n')
            for k,v in self.lsv_base_GT_distr.items():
                #out.write(','.join([k,str(v),str(self.lsv_comm_GT_distr[k]),str(self.lsv_base_no_GT_distr[k]),str(self.lsv_comm_no_GT_distr[k])])+'\n')
                 tmp_base_GT = v
                 tmp_comm_GT = self.lsv_comm_GT_distr[k]
                 tmp_base_no_GT = self.lsv_base_no_GT_distr[k]
                 tmp_comm_no_GT = self.lsv_comm_no_GT_distr[k]
                 GT_recall = v/float(self.baseSize)
                 GT_precision = self.lsv_comm_GT_distr[k]/float(self.commSize)
                 GT_F1 = str(2*GT_recall*GT_precision/(GT_recall+GT_precision))
                 no_GT_recall = self.lsv_base_no_GT_distr[k]/self.baseSize
                 no_GT_precision = self.lsv_comm_no_GT_distr[k]/self.commSize
                 no_GT_F1 = str(2*no_GT_recall*no_GT_precision/(no_GT_recall+no_GT_precision))
                 out.write(','.join([k,str(tmp_base_GT),str(tmp_comm_GT),str(tmp_base_no_GT),str(tmp_comm_no_GT),str(GT_recall),str(GT_precision),GT_F1,str(no_GT_recall),str(no_GT_precision),no_GT_F1])+'\n')
