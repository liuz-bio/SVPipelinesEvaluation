...
class VCFRead():
    def __init__(self, platform=None, depth=None, svType=None, vcfFile=None,outFiles=None,
                 ref="data/hs37d5.fa",mapTool=None, svTool=None, RE_nus=list(range(2,21)), nux=None):
        ...
        self.tmpFileName = 'tmp.%s.%s.%s.%s.%s.%s.vcf'%(str(RE_nu),str(self.nux),platform,depth,mapTool,svTool)
        self.writetmp()
        ...
        self.SvLines = dict(zip(RE_nus, [[] for i in range(2,20)]))
        print(self.vcfFile)
        if svType=='BND':
    ...
    def OutPut(self):
        for RE_nu, outFile in self.outFiles.items():
            os.makedirs('/'.join(outFile.split('/')[:-1]), exist_ok=True)
            print('/'.join(outFile.split('/')[:-1]))
            out = open(outFile,"w")
            for line in self.vcfHead+self.SvLines[RE_nu]:
                out.write(line+'\n')
            out.close()
    ...
    def run(self):
        ...
        for line in self.vcfIn:
            ...
            if (line_SVTYPE != self.svType) or \
                ...
                line_RE = self.getRE(line)
                if self.svType == 'DUP':
                    if tmpGT == "None/None" :
                        for RE_nu in self.SvLines:
                            if line_RE>=RE_nu:
                                tmpSample = tmpSample.replace("None/None",'0/1')
                                self.SvLines[RE_nu].append('\t'.join([self.getChrID(line),
                                                           self.getPos(line),
